...
class VideoTool:
    ...
    def videosave_rec(self):
        if self.out is not None:
            if self.start_time + 3 > datetime.now().minute:
                self.out.write(self.get_frame(640, 480, None, False))
            else:
                self.videosave_restart()
        else:
            self.videosave_start()

    def videosave_start(self):
        if not os.path.exists("saved_videos/"):
            os.mkdir("saved_videos/")
        self.current_videofile_name = ("saved_videos/Cam_"
                                       + str(self.camera_number + 1)
                                       + "_"
                                       + str(datetime.now().strftime('%d_%m_%y_%H_%M_%S'))
                                       + ".avi")
        self.out = cv2.VideoWriter(self.current_videofile_name,
    ...
    def videosave_restart(self):
        self.videosave_stop()
        self.videosave_start()

    def play_video(self):
        if os.path.exists(self.current_videofile_name):
            # self .current_videofile_name_buffer = r"C:\Users\t333g\PycharmProjects\SecuritySystem/" + self.current_videofile_name
            subprocess.Popen(["C:\Program Files (x86)\K-Lite Codec Pack\MPC-HC64\mpc-hc64.exe", self.current_videofile_name])
            # self.videosave_restart()
